...
Every service is curated rather than a mirror of the plugin's internals. Menu openers, editor flows,
administrative writes and raw configuration rows are left out on purpose: a public contract cannot be
broken later, so it holds what an integration genuinely needs and nothing that only made sense inside

...
Three modules with the same shape: none of them creates a real entity, all three are drawn straight to
a client, and all three own the lifetime of what they show.
...
All three are reachable from a [sequence](/docs/exylialib/sequences) line, which is how every cosmetic
...
A body that comes apart. The player is cut into its six parts — head, torso, two arms, two legs — each
one wearing the colours of their own skin, and then those parts are moved as a single piece of
choreography: thrown, opened out, spun, flattened, spelled into letters, or carried off whole.
...
The head takes `{victim}`, `{killer}`, or a base64 texture, exactly as `[NPC]` does. If what died was
...
<Callout type="tip" title="`detail` is the only expensive parameter here">
A part is drawn as a `detail` × `detail` grid, so six parts cost `6 × detail²` displays: `detail:1` is
...
<Callout type="note" title="A pose is choreography, not physics">
Every one of these is solved in advance and handed to the client as poses, so what separates them is
what the file wanted to say, never what the server could afford. Nothing here is simulated, and

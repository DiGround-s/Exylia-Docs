...
A plugin with a long name can publish the same placeholders under a **second identifier**:

```java
Placeholders.identifier(this, "practice");
```

`%exyliapracticecore_stats_kills%` and `%practice_stats_kills%` then mean the same thing. Nothing is
renamed, so configs already written the long way go on working. The extra identifier is one word
without `_` or `%`, since PlaceholderAPI reads everything up to the first underscore as the
identifier; the call is safe before the placeholders are registered and safe to repeat.

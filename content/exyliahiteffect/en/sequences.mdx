...
| `face` | Turns it to face whoever landed the hit. |
...
It is a display, not an entity: nothing can hit it, loot it or walk into it. None of the shipped hit
effects uses it — a body left behind on every blow is a kill effect, not a hit effect — but the step
is there if you want one.
...
<Callout type="warning" title="Cost is per line, per hit — not per kill">
A display is cheap to animate — the client does it — but every line is still a packet per viewer, on
every blow of every fight. The shipped effects are two to five lines and under six tenths of a
second; that is the budget, not a coincidence.
</Callout>
